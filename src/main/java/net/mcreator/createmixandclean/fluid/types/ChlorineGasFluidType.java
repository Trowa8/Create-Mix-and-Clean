...
public class ChlorineGasFluidType extends FluidType {
	public ChlorineGasFluidType() {
		super(FluidType.Properties.create().fallDistanceModifier(0F).canExtinguish(true).supportsBoating(true).canHydrate(true).motionScale(0.0049D).density(3).viscosity(200).sound(SoundActions.BUCKET_FILL, SoundEvents.BUCKET_FILL)
	...
	public static void registerFluidTypeExtensions(RegisterClientExtensionsEvent event) {
		event.registerFluidType(new IClientFluidTypeExtensions() {
			...
			public void modifyFogRender(Camera camera, FogRenderer.FogMode mode, float renderDistance, float partialTick, float nearDistance, float farDistance, FogShape shape) {
				...
				RenderSystem.setShaderFogEnd(Math.min(20f, renderDistance));
			}
		}, CreateMixAndCleanModFluidTypes.CHLORINE_GAS_TYPE.get());
	}
}